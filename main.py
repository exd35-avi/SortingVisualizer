...
# #DRAWING THE ARRAY ELEMENTS AS BARS
def drawData(data, colorArray):
    canvas.delete("all")     # every time when the swap happens the rectangle should be redrawn
    canvas_width = 800
    canvas_height = 400
    x_width = canvas_width / (len(data) + 1)
    offset = 4
    spacing = 2
    normalizedData = [i / max(data) for i in data]      # normalization is done
...
def generate():
    global data     # explicitly done as global
...
# where all the functional button will be shown
UI_frame = Frame(window, width= 900, height=300, bg=WHITE)
UI_frame.grid(row=0, column=0, padx=10, pady=5)

# what type of sorting you want to select
...
# where the visualization will take place
...
# current array button with the function Generate_array
